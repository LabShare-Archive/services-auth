...
describe("Services-Auth", () => {
  ...
  describe("when authorizing users", () => {
    ...
    beforeEach(done => {
      const loggerMock = jasmine.createSpyObj("logger", [
        "error",
        "info",
        "warn"
      ]);
    ...
    describe("when checking if the token is revoked", () => {
      beforeEach(() => {
        services.config(
          servicesAuth({
            ...
            isRevokedCallback: (req, payload, cb) => {
              cb(null, true);
            }
          })
        );
      ...
      it("blocks access if the token is revoked", async () => {
        const token = createToken(1, "read:books");

        try {
          await request
            .get("/api-package-1-namespace/books")
            .set("Authorization", "Bearer " + token)
            .expect("got books");

          throw new Error("Should have blocked a revoked token");
        } catch (error) {
          expect(error.message).toContain("The token has been revoked");
        }
      });
    });
    ...
    describe("when authorizing with Resource Scopes", () => {
      ...
      it("allows users with the appropriate scopes", async () => {
        const token = createToken(1, "read:books");
      ...
      it("blocks users without the appropriate scopes", async () => {
        const token = createToken(1, "read:books");
    ...
    describe("with legacy authentication check", () => {
      ...
      it("allows authenticated users", done => {
        ...
        request
          .get("/api-package-1-namespace/staff/task")
          .set("Authorization", "Bearer " + token)

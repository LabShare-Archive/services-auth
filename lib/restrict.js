 ...
 */
module.exports = function restrict(route, authUrl) {
    ...
    return function restrictMiddleware(req, res, next) {
        ...
        let token = req.headers['auth-token'] || req.cookies.authToken;

        // Don't process unauthenticated or already authenticated requests
        if (!token || req.user || !route.accessLevel || route.accessLevel === 'public') {
            next();
            return;
        }

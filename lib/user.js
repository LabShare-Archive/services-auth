 ...
 * @returns {Function}
 */
module.exports = function authUser({authUrl, tenant}) {
    assert.ok(isString(authUrl), 'services-auth: authUser requires an "authUrl"');
    assert.ok(isString(tenant), 'services-auth: authUser requires a "tenant"');
    ...
    return ({authToken = ''}, callback) => {
        ...
        const userProfileUrl = `${authUrl}/auth/${tenant}/me`;

        ...
        if (accessLevelDeclarations.hasOwnProperty(level)) {
            if (typeof accessLevelDeclarations[level] === 'string') {
                if (accessLevelDeclarations[level] === '*') {
                    ...
                    throw new Error(`Access Control Error: Could not parse "${accessLevelDeclarations[level]}"' as access definition for level "${level}"`);
                }
            } else {
                resultBitMask = 0;
                for (role in accessLevelDeclarations[level]) {
                    if (userRoles.hasOwnProperty(accessLevelDeclarations[level][role])) {
                        ...
                        throw new Error(`Access Control Error: Could not find role "${accessLevelDeclarations[level][role]}"
